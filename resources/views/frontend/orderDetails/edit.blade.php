    ...
    <div class="card-body">
        <form method="POST" action="{{ route("frontend.order-details.update", [$orderDetail->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('order') ? 'is-invalid' : '' }}" name="order_id" id="order_id">
                    @foreach($orders as $id => $entry)
                        <option value="{{ $id }}" {{ (old('order_id') ? old('order_id') : $orderDetail->order->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }} - {{ $id }}</option>
                    @endforeach
                </select>
                @if($errors->has('order'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('quantity') ? 'is-invalid' : '' }}" type="number" name="quantity" id="quantity" value="{{ old('quantity', $orderDetail->quantity) }}" step="1">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('total_cost') ? 'is-invalid' : '' }}" type="number" name="total_cost" id="total_cost" value="{{ old('total_cost', $orderDetail->total_cost) }}" step="0.01">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('colors') ? 'is-invalid' : '' }}" type="text" name="colors[]" id="colors" value="{{ old('colors', $orderDetail->colors) }}" placeholder="add colors ..." data-role="tagsinput">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('size') ? 'is-invalid' : '' }}" type="text" name="size" id="size" value="{{ old('size', $orderDetail->size) }}">
                @if($errors->has('size'))
                    <div class="invalid-feedback">
                        {{ $errors->first('size') }}
                    </div>
