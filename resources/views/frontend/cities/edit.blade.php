...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("frontend.cities.update", [$city->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="predictions">{{ trans('cruds.city.fields.predictions') }}</label> 
                <input type="text" class="form-control {{ $errors->has('predictions') ? 'is-invalid' : '' }}" name="predictions[]" value="{{ $city->predictions }}" placeholder="add predictions ..." data-role="tagsinput">
                @if($errors->has('predictions'))
                    <div class="invalid-feedback">
                        {{ $errors->first('predictions') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.city.fields.predictions_helper') }}</span>
            </div>
            <div class="form-group">
