...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("frontend.countries.update", [$country->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="predictions">{{ trans('cruds.country.fields.predictions') }}</label> 
                <input type="text" class="form-control {{ $errors->has('predictions') ? 'is-invalid' : '' }}" name="predictions[]" value="{{ $country->predictions }}" placeholder="add predictions ..." data-role="tagsinput">
                @if($errors->has('predictions'))
                    <div class="invalid-feedback">
                        {{ $errors->first('predictions') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.country.fields.predictions_helper') }}</span>
            </div>
            <div class="form-group">
